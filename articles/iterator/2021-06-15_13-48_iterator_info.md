...
# Who to write an iterator?
Now that, hopefully, we can understand what an iterator is, we ca take a look at an example that implements an iterator over a singly linked list. 
...
Note that in this exercise will be using smart pointer (specifically unique_ptr) so that we don't have to wary about freeing the memory manually for the stored data. If you're not familiar with smart pointers there is a lot of great information out there about them.
...
We then need to write a class that will represent our list and because we don't care to much about what type our data is will create a template class that can accept all sorts of data.
...
As you might have noticed I've added a head which will always be the first node in the list and a size where we keep track of the number of elements in the list. Keeping track of the number of elements in the list is useful to find out if the list is empty without having to go thorough the list, for this we can build these two methods:
...
Adding to the back involves going though the list because we can't access any elements directly except for the head:
...
Aside from the rule of 5 semantics I would like to add a special constructor that can make like easier for most people to use our structure. The constructor that will create a list of elements from an initializer list.
...
Now, before we move on to actually implement the iterator let's talk about what would be your first thought on just printing each element in the list. the first go to would be to build up a simple method that will go trough the list and print each element since we already know how to traverse it.
...
I'll add that in order to successfully convert a node to string I've added a conversion operator to the Node structure that will use a custom to_string function which can convert anything convertible to string:
...
Now we can use our iterator to traverse the list like we would normally do with a simple vector:
...
At this point the iterator we created cannot modify the list and all elements return are const, thus this can be considered and const iterator. But we can make a few simple modifications to the iterator class so that we could also insert an element while we iterate:

We have to add LinkedList as a friend class to the Iterator so that it can access it's private members.
...
As a final thing we will be add a new method that inserts an new element before a specified position:
...
# Conclusions
* Building an iterator will simplify the code for the given data structure simply because the iterator is responsible for traversing the structure and not the structure itself, the structures only responsibility is to supply the start and the end for the iterator.
